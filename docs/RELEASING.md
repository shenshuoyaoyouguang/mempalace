...
### Verify `mempalace-mcp` / `mempalace-hook` entry point alignment

The plugin configs reference `mempalace-mcp` as the MCP server command and
`mempalace-hook` as the hook command. These resolve to entry points declared
under `[project.scripts]` / `[project.gui-scripts]` in `pyproject.toml`. If
these disagree, `pip install mempalace` ships plugin configs pointing at
binaries that were never installed — exactly what broke v3.3.2 for the MCP
path ([#1093](https://github.com/MemPalace/mempalace/issues/1093)).

```bash
grep -r -E "mempalace-mcp|mempalace-hook" pyproject.toml .claude-plugin .codex-plugin
```

Expected on a healthy `develop` — one line per relevant file:

```
pyproject.toml:mempalace-mcp = "mempalace.mcp_server:main"
pyproject.toml:mempalace-hook = "mempalace.hook_entry:main"
...
.claude-plugin/hooks/hooks.json:            "command": "mempalace-hook --hook stop --harness claude-code"
.claude-plugin/hooks/hooks.json:            "command": "mempalace-hook --hook precompact --harness claude-code"
.codex-plugin/hooks.json:            "command": "mempalace-hook --hook session-start --harness codex"
.codex-plugin/hooks.json:            "command": "mempalace-hook --hook stop --harness codex"
.codex-plugin/hooks.json:            "command": "mempalace-hook --hook precompact --harness codex"
```

If `pyproject.toml` has no match for either referenced entry point, **stop** —
the entry point is missing and any fresh `pip install` will ship a broken
plugin config.
